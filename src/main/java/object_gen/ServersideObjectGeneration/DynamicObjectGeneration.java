...
public class DynamicObjectGeneration {
	...
	public void generateObject(String data_store,String Host, String Port, String setSchema,String database , String table, String className, String packageName,String isRelationship, String relationshipType, String 
		...
		try{
			if (data_store.toLowerCase().equals("mongodb")){
		        ...
		        GettingMetadata_New gmd= new GettingMetadata_New();
			      gmd.generateMongoEntity(data_store,Host, Port,setSchema, database, table,className, packageName, CLASSES_PATH, CLASSES_OUTPUT,isRelationship,relationshipType,
		        ...
		        if (!node.exists()) 
		    		node.mkdir();
		        GettingMetadata_New gmd= new GettingMetadata_New();
		        gmd.generateCassandraEntity(data_store,Host, Port,setSchema, database, table,className, packageName, CLASSES_PATH, CLASSES_OUTPUT,isRelationship,relationshipType,
		        		attributeName,relationshipTable,relationshipClass,relationshipDATA_STORE,relationshipHost,relationshipPort);
			}
	...
	public void generateObject(String data_store,String Host, String Port,String setSchema, String database , String packageName,String OUTPUT_PACKAGE){
		try{
				...
				if (data_store.toLowerCase().equals("mongodb")){
			      ...
			      connnectionProperties.put("kundera.nodes", Host);
			        ...
			        for(int i=0;i<noOfCollections;i++){
				       ...
				       gmd.generateMongoEntity(data_store,Host, Port, setSchema,database, collectionName, collectionName, packageName, CLASSES_PATH, CLASSES_OUTPUT, "false",
				    		   "", "", "", "", "", "", "");
			        }
			    	...
			    	EntityManagerFactory emf = Persistence.createEntityManagerFactory("cassandra_pu",connnectionProperties);
				        ...
				        for(int i=0;i<colfList.size();i++){
				        	...
				        	gmd.generateCassandraEntity(data_store,Host, Port, setSchema,database, colfList.get(i), colfList.get(i), packageName, CLASSES_PATH, OUTPUT_PACKAGE, "false",
				        			"","", "","","", "", "");
